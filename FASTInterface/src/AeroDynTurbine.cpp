...
/*! @brief Begin updating the model's state by first setting the nacelle's states in the future. We can't know where the blade nodes will be 
...
//! @brief this takes the substeps needed to reach within global_timestep of dvr_time_next
...
//! @brief Performs the predictor-corrector method, and uses the input-output solver for each corrector iteration (when added mass is enabled)
...
/*! 
 * @brief Solves for the correct acceleration inputs at global_time_next and then calculates the outputs at that same time.
 * 
 * 
 */
AeroDynTurbine::NacelleReactionLoads_Vec AeroDynTurbine::CalcOutputs_And_SolveInputs()
{
	...
	//! @todo initialize perturb vector somewhere in the constructor
	...
	*/
	...
	// The previous call to Dvr_CalcOutput set the nacAccels... variable, and since the solver 
	// settled on the values used at that last call, consider them the correct accelerations at dvr_time_next; 
	// therefore, save them.
	nacelleMotion_at_dvr_time_next.acceleration = nacAccels_at_dvr_time_next.acceleration;
	nacelleMotion_at_dvr_time_next.angularAcc = nacAccels_at_dvr_time_next.rotation_acceleration;

	return result;
}

/*!
 * @brief Transfers the outputs from one module to the inputs of the other, then these
 ...
 * @param y the outputs generated by the inputs u
 * @param u the inputs (this is the actual input to the mathematical residual function)
 * @return \f$u - U(Y(u))$
*/
...
AeroDynTurbine::NacelleAccelerations AeroDynTurbine::Dvr_CalcOutput(const Vector3d& nacelle_force, const Vector3d& nacelle_moment)
{
	...
	// Call the function with these extrapolated inputs
